...
func f38(b bool) {
	...
	if b {
		...
		case *fi38(2) = <-fc38(): // ERROR "live at call to fc38:( \.autotmp_[0-9]+)+$" "live at call to fi38:( \.autotmp_[0-9]+)+$" "live at call to selectrecv:( \.autotmp_[0-9]+)+$"
			printnl()
		case *fi38(3), *fb38() = <-fc38(): // ERROR "live at call to fb38:( \.autotmp_[0-9]+)+$" "live at call to fc38:( \.autotmp_[0-9]+)+$" "live at call to fi38:( \.autotmp_[0-9]+)+$" "live at call to selectrecv2:( \.autotmp_[0-9]+)+$"
			printnl()
		}
		printnl()
	}
