...
func (t *Named) expand() {
	if t.resolve != nil {
		t.once.Do(func() {
			...
			// (necessary because go/types expects the receiver type for
			// methods on defined interface type to be the Named rather than
			// the underlying Interface), maybe it should just handle
			// calling SetTParams, SetUnderlying, and AddMethod instead?
			// Those methods would need to support reentrant calls though.
			// It would also make the API more future-proof towards further
			// extensions (like SetTParams).
...
func (t *Named) NumMethods() int { t.expand(); return len(t.methods) }

// Method returns the i'th method of named type t for 0 <= i < t.NumMethods().
func (t *Named) Method(i int) *Func { t.expand(); return t.methods[i] }
