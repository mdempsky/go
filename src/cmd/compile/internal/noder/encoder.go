...
func (pw *pkgEncoder) dump(data io.Writer) {
	writeUint32 := func(x uint32) {
		assert(binary.Write(data, binary.LittleEndian, x) == nil)
	}
	...
	for _, elems := range &pw.elems {
		for _, elem := range elems {
			io.WriteString(data, elem)
		}
	}
}

func (pw *pkgEncoder) newEncoder(k reloc) encoder {
	idx := len(pw.elems[k])
