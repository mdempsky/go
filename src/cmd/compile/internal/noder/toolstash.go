...
// -cmp when compared against the legacy frontend behavior, but
// can be removed after that's no longer a concern.

// quirksMode controls whether behavior specific to satsifying
// toolstash -cmp is used.
func quirksMode() bool {
	// Currently, pkgbits doesn't try to be compatible with
	// -d=inlfuncswithclosures=1 anyway, so we overload this as a flag
	// for enabling quirks mode.
...
func importedObjsOf(curpkg *types2.Package, info *types2.Info, noders []*noder) []types2.Object {
	// This code is very complex because it matches the precise order
	// that typecheck recursively and repeatedly traverses the IR.
